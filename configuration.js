...
const additionConfigPath = path_1.join(__dirname,'../','../','../','tsconfig.json');
const additionConfig = require(additionConfigPath);
...
function readConfig(cwd, ts, rawApiOptions) {
    ...
    if (!skipProject) {
        ...
        }else if(process.env.TS_NODE_ROOT){
            const nested = (0, path_1.join)(process.env.TS_NODE_ROOT, 'tsconfig.json');
            configFilePath = fileExists(nested) ? nested : process.env.TS_NODE_ROOT;
        }
        else {
